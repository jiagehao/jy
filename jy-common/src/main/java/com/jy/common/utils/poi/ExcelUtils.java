...
import com.jy.common.utils.reflect.ReflectUtils;
...
public class ExcelUtils<T>
{
     ...
     *
     * @param is 输入流
     * @param num Excel表格从第几行开始读起，默认第二行
     * @return 转换后集合
     */
    public List<T> importExcel(InputStream is,int num) throws Exception
    {
        return importExcel(StringUtils.EMPTY, is,num);
    }
     ...
     * @return 转换后集合
     */
    public List<T> importExcel(String sheetName, InputStream is,int num) throws Exception
    {
        boolean isnum = false;
        if(num>0){
            isnum = true;
...
//        }
        for (int o = 0; o < wb.getNumberOfSheets(); o++) {
                        ...
                        if(column==10){
                            if(val != null){
                                if(sheet.getRow(10) != null){
                                    if(sheet.getRow(column).getPhysicalNumberOfCells()>24){
