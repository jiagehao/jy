...
(function ($) {
    $.extend({
    	...
    	table: {
            ...
            init: function(options) {
                ...
                $('#' + options.id).bootstrapTable({
                    ...
                    editable:options.editable	,						 //打开行内编辑
                    onEditableSave:options.onEditableSave,               // 行编辑保存函数
                });
            },
            // 查询条件
            ...
            // 导入数据
            importExcel: function(formId,clazz,num) {
            	...
            	layer.open({
            		...
            		btn1: function(index, layero){
            			...
            			if(num==1&&clazz==''||clazz==null||clazz==undefined){
                            formData.append("updateSupport", $("input[name='updateSupport']").is(':checked'));
                            formData.append('clazzName',$("input[name='clazzName']").val());
                            formData.append('subjectId',$("#subjectID").find("option:selected").val());
                            formData.append('subjectName',$("#subjectID").find("option:selected").text());
                            formData.append('begintime',$("input[name='begintime']").val());
                            formData.append('endtime',$("input[name='endtime']").val());
                            formData.append('headmasterName',$("input[name='headmasterName']").val());
						}else if(num==0&&clazz==''||clazz==null||clazz==undefined){
                            formData.append("updateSupport", $("input[name='updateSupport']").is(':checked'));
                            formData.append('clazzName',$("input[name='clazzName']").val());
                            formData.append('subjectId',$("#subjectID").find("option:selected").val());
                            formData.append('subjectName',$("#subjectID").find("option:selected").text());
                            ...
                            formData.append('endtime',$("input[name='endtime']").val());
                            formData.append('headmasterName',$("input[name='headmasterName']").val());
                            formData.append('assistantname',$("input[name='assistantname']").val());
                            formData.append('lecturername',$("input[name='lecturername']").val());
						}
            			$.ajax({
			...
			// 导入数据
            importFormExcels: function(formId,clazz) {
            	var currentId = $.common.isEmpty(formId) ? 'importForm' : formId;
            	$.form.reset(currentId);
            	layer.open({
            		type: 1,
            		...
            		btn1: function(index, layero){
            			...
            			var formData = new FormData();
            			formData.append("file", $('#file')[0].files[0]);
						formData.append('clazzId',$("input[name='clazzId']").val());
						formData.append('updateSupport',$("input[name='updateSupport']").val());
            			$.ajax({
            				url: $.table._option.importUrl,
            				data: formData,
            				cache: false,
            ...
            // 导入数据
            importExcelCompanyName: function(formId,clazz) {
                var currentId = $.common.isEmpty(formId) ? 'importForm' : formId;
                $.form.reset(currentId);
                layer.open({
                    type: 1,
                    ...
                    shadeClose: true,
                    btn1: function(index, layero){
                        var file = layero.find('#files').val();
                        if (file == '' || (!$.common.endWith(file, '.xls') && !$.common.endWith(file, '.xlsx'))){
                            $.modal.msgWarning("请选择后缀为 “xls”或“xlsx”的文件。");
                            return false;
                        }
                        ...
                        // formData.append('updateSupport',$("input[name='updateSupport']").val());
                        $.ajax({
                            url: $.table._option.importExcelCompanyName,
                            data: formData,
                            cache: false,
                            contentType: false,
                            processData: false,
        ...
        operate: {
			...
			openJoin:function(url,title) {
                top.layer.open({
                    type: 2,
                    title: '<div  style="margin-left: 15px; margin-right: 5px;font-weight:bold">'+
            ...
            openStudentInfo: function(id) {
                ...
                if($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
                    ...
                    //st
                    var url = $.table._option.infoUrl.replace("{stuId}", row[$.table._option.uniqueId]);
                    $.modal.open("记录" + $.table._option.modalName, url);
                } else {
                    $.modal.open("记录" + $.table._option.modalName, $.operate.studentInfoUrl(id));
                }
            },


            // 添加学生跟紧信息
            addStudentInfo: function(id) {
                var scollPostion = $('#bootstrap-table').bootstrapTable('getScrollPosition');
                ...
                if($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
                    ...
                    $.modal.open("添加"+ $.table._option.modalName+"跟踪记录", url);
                } else {
                    $.modal.open("添加"+ $.table._option.modalName+"跟踪记录", $.operate.addStudentInfoUrl(id));
                }
            },

            // 添加过程服务跟踪或者违纪记录
            addServerInfo: function(id,isfoul) {
                var scollPostion = $('#bootstrap-table').bootstrapTable('getScrollPosition');
                console.log(scollPostion)
                if($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
                    var row = $('#' + $.table._option.id).bootstrapTreeTable('getSelections')[0];
                	...
                	var infoMsg=isfoul=='0'?"跟踪记录":'违纪记录';
                    $.modal.open("添加"+ $.table._option.modalName+infoMsg, $.operate.addServerInfoUrl(id,isfoul));
                }
            },
            ...
            // 学员记录信息
            studentInfoUrl: function(id) {
                
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = $.table._option.infoUrl.replace("{stuId}", id);
                } else {
                    ...
                    url = $.table._option.studentInfoUrl.replace("{stuId}", id);
                }
                return url;
            },

            // 学员记录信息
            addStudentInfoUrl: function(id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = $.table._option.addInfoUrl.replace("{stuId}", id);
                } else {
                    var id = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
 			...
 			// 学员过程服务跟踪或者违纪记录信息
            addServerInfoUrl: function(id,isfoul) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url=$.table._option.addInfoUrl;
                    url=url.replace("{stuId}", id);
                    url=url.replace("{isfoul}", isfoul);
                } else {
                    var id = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
                    ...
                    url=$.table._option.addInfoUrl;
                    url.replace("{stuId}", id);
                    url.replace("{isfoul}", isfoul);
                }
                return url;
            },

            // 学员记录信息
            addStudentInfoUrlLayer: function(id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = addUrl.replace("{stuId}", id);
                } else {
                    var id = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
...
var OptionUtils = function(selectId,url,textID,textName){
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            for(var i=0;i<data.length;i++){
                option += "<option value='"+data[i][textID]+"'>"+data[i][textName]+"</option>";
            }
            $("#"+selectId).empty();
