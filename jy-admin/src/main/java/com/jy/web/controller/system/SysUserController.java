...
import com.jy.framework.shiro.service.SysPasswordService;
import com.jy.framework.util.ShiroUtils;
import com.jy.system.domain.SysUser;
import com.jy.system.service.ISysPostService;
...
//            return error("不允许修改超级管理员用户");
//        }
        if(user.getSubjectId().length()>32){
            if(user.getSubjectId().split(",").length>1){
                user.setSubjectId(user.getSubjectId().split(",")[1]);
            }else{
                user.setSubjectId("");
            }
        }
